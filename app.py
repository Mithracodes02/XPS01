...
# Check for vamas dependency safely
try:
    import vamas
    HAS_VAMAS = True
except ImportError:
...
if uploaded_file is not None:
    try:
        ...
        if file_ext in ["vcm", "vms"]:
            if not HAS_VAMAS:
                st.error("The `vamas` library is missing. Please ensure `vamas` is listed in your `requirements.txt`.")
            else:
                ...
                if hasattr(block, "corresponding_variables") and len(block.corresponding_variables) > 0:
                    ...
                    for attr in ["y_values", "array", "values", "data", "y"]:
                        if hasattr(var0, attr) and getattr(var0, attr) is not None:
                            y_vals = list(getattr(var0, attr))
                            break
                    if not y_vals:
                        try:
                            y_vals = list(var0)
                ...
                if not y_vals:
                    for attr in ["y", "y_array", "y_values"]:
                        if hasattr(block, attr) and getattr(block, attr) is not None:
                            y_vals = list(getattr(block, attr))
                            break

                # Extract X (Binding Energy)
                x_vals = []
                for attr in ["abscissa", "x", "x_array", "x_values"]:
                    if hasattr(block, attr) and getattr(block, attr) is not None:
                        x_vals = list(getattr(block, attr))
                        break

                if not x_vals and len(y_vals) > 0:
                    x_start = getattr(block, "abscissa_start", 0)
                    x_step = getattr(block, "abscissa_increment", 1)
                    x_vals = [x_start + i * x_step for i in range(len(y_vals))]

                if len(x_vals) > 0 and len(y_vals) > 0:
                    min_len = min(len(x_vals), len(y_vals))
                    df = pd.DataFrame({"BE": x_vals[:min_len], "Intensity": y_vals[:min_len]})

        # Parse Excel (.xlsx, .xls)
        elif file_ext in ["xlsx", "xls"]:
            raw_df = pd.read_excel(uploaded_file)
            if not raw_df.empty:
                col_be = st.selectbox("Select Binding Energy (eV) Column", raw_df.columns, index=0)
        ...
        # Parse Text / CSV
            ...
            raw_df = pd.read_csv(uploaded_file, delimiter=delimiter, engine="python", encoding="latin1")
            if not raw_df.empty:
                col_be = st.selectbox("Select Binding Energy (eV) Column", raw_df.columns, index=0)
                col_int = st.selectbox("Select Intensity Column", raw_df.columns, index=min(1, len(raw_df.columns) - 1))
                df = raw_df[[col_be, col_int]].dropna().rename(columns={col_be: "BE", col_int: "Intensity"})
        ...
        if df is None or df.empty:
            st.error("No valid numeric data could be read from this file. Please check your file formatting or column selections.")
        else:
            df["BE"] = pd.to_numeric(df["BE"], errors="coerce")
            df["Intensity"] = pd.to_numeric(df["Intensity"], errors="coerce")
            df = df.dropna().sort_values(by="BE", ascending=True).reset_index(drop=True)

            if df.empty or len(df) < 2:
                st.error("Insufficient valid data points found after filtering NaNs.")
            else:
