...
# Reference Core Level Binding Energies (eV)
XPS_DATABASE = {
    "C 1s": 284.8,
    "C-O": 286.5,
    "C=O": 288.0,
...
uploaded_file = st.sidebar.file_uploader(
    "Upload XPS Data File (.vcm, .vms, .csv, .xlsx, .txt)",
    type=["vcm", "vms", "csv", "xlsx", "xls", "txt"],
)

st.sidebar.header("2. Elemental Identification")
selected_elements = st.sidebar.multiselect(
    "Select Target Elements/Core Levels",
...
st.sidebar.header("3. Deconvolution Constraints")
...
if uploaded_file is not None:
    try:
        ...
        if file_ext in ["vcm", "vms"]:
            if not HAS_VAMAS:
                st.error(
                    "The `vamas` library is missing. Please ensure `vamas` is listed in your `requirements.txt`."
                )
            else:
                ...
                df = pd.DataFrame(
                    {
                        "BE": block.abscissa,
                        "Intensity": block.corresponding_variables[0].array,
                    }
                )
        ...
        # Parse Excel (.xlsx, .xls)
            ...
            col_be = st.selectbox(
                "Select Binding Energy (eV) Column", raw_df.columns, index=0
            )
            ...
            df = raw_df[[col_be, col_int]].dropna().rename(
                columns={col_be: "BE", col_int: "Intensity"}
            )
        ...
        # Parse Text / CSV
            ...
            raw_df = pd.read_csv(
                uploaded_file, delimiter=delimiter, engine="python"
            )
            col_be = st.selectbox(
                "Select Binding Energy (eV) Column", raw_df.columns, index=0
            )
        ...
        if df is not None and not df.empty:
            ...
            baseline = np.linspace(
                df["Intensity"].iloc[0], df["Intensity"].iloc[-1], len(df)
            )
            df["Corrected_Intensity"] = np.maximum(
                0, df["Intensity"] - baseline
            )
            ...
            peak_indices, _ = find_peaks(
                y_data, prominence=prominence * np.max(y_data)
            )
            ...
            for be in detected_be:
                ...
                identifications.append(
                    {
                        "Detected BE (eV)": round(be, 2),
                        "Matched Core Level": matched,
                    }
                )

            # Interactive Fit Inputs
            st.subheader("Peak Fitting Constraints")
            ...
            for idx in range(int(num_peaks)):
                with fit_cols[idx]:
                    ...
                    def_center = (
                        float(detected_be[idx])
                        if idx < len(detected_be)
                        else float(np.mean(x_data))
                    )
                    center = st.number_input(
                        f"Center BE (eV) #{idx+1}", value=round(def_center, 2)
                    )
                    amp = st.number_input(
                        f"Amplitude #{idx+1}",
                        value=float(np.max(y_data) / num_peaks),
                    )
                    sigma = st.number_input(
                        f"Sigma #{idx+1}", value=1.0, min_value=0.1, step=0.1
                    )
            ...
            ax.plot(
                x_data, df["Intensity"], "k.", alpha=0.3, label="Raw Spectrum"
            )
            ax.plot(
                x_data,
                fit_y + baseline,
            ...
            for i in range(0, len(popt), 3):
                ...
                ax.plot(
                    x_data,
                    component_y + baseline,
                    label=f"Peak @ {center:.2f} eV",
                )
                ax.fill_between(
                    x_data, baseline, component_y + baseline, alpha=0.2
                )
            ...
            for p in peak_summary:
                p["Area (%)"] = (
                    round((p["Area (a.u.·eV)"] / total_area) * 100, 2)
                    if total_area > 0
                    else 0
                )
            ...
            ax.invert_xaxis()  # XPS Binding Energy decreasing scale
            ...
            with col_l:
                ...
                st.dataframe(
                    pd.DataFrame(identifications), use_container_width=True
                )

            with col_r:
                st.subheader("Peak Quantification")
                df_peaks = pd.DataFrame(peak_summary)
                st.dataframe(df_peaks, use_container_width=True)

            # Export Excel Report with Image
            ...
            with pd.ExcelWriter(
                ...
                df_peaks.to_excel(
                    writer, sheet_name="Peak Summary", index=False
                )
                pd.DataFrame(identifications).to_excel(
                    writer, sheet_name="Identifications", index=False
                )
    ...
    st.info("Upload a `.vcm`, `.vms`, `.csv`, or `.xlsx` XPS file to begin.")
